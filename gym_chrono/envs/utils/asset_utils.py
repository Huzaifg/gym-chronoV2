# Description: Utility functions for GatorGym environment
...
class AssetMesh():
    """ Class to hold mesh data attached to a Chrono body.  """
    ...
    def __init__(self, filename, bounding_box=None):
        ...
        if bounding_box == None:
            self.bounding_box = self.CalcBoundingBox()
        else:
    ...
    def CalcBoundingBox(self):
        ...
        minimum = chrono.ChVectorD(min(vertices, key=lambda x: x.x).x, min(
            vertices, key=lambda x: x.y).y, 0)
        maximum = chrono.ChVectorD(max(vertices, key=lambda x: x.x).x, max(
            vertices, key=lambda x: x.y).y, 0)
...
class Asset():
    ...
    def Transform(self, pos, scale=1, ang=0):
        ...
        self.mesh.mesh.Transform(chrono.ChVectorD(
            0, 0, 0), chrono.ChMatrix33D(scale))
        self.mesh.body.SetRot(chrono.Q_from_AngAxis(
...
class AssetList():
    def __init__(self, b1=0, b2=0, r1=0, r2=0, r3=0, r4=0, r5=0, t1=0, t2=0, t3=0, c=0):
        ...
        self.assets.append(Asset(AssetMesh(
            "sensor/offroad/tree3.obj", chrono.ChVectorD(4.7282, 4.67921, 0)), 5, 5, t3))
        self.assets.append(
            ...
            for _ in range(asset.num):
                ...
                while True:
                    ...
                    pos = self.CalcRandomPose(
                        terrain, length, width, offset=-random.random() * .5)
                    if should_scale:
                        scale = self.map(
                            random.random(), asset.min_scale, asset.max_scale)
                    else:
                        scale = 1
